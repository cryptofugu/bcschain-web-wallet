import bcsJsLib from 'bcsjs-lib'
import btcApp from '@ledgerhq/hw-app-btc'
import transportU2f from '@ledgerhq/hw-transport-u2f'
import BigNumber from 'bignumber.js'
import SafeBuffer from 'safe-buffer'
import OPS from 'bcs-opcodes'
...
export default class Ledger {
  ...
  static async connect() {
    const bcs = new btcApp(await transportU2f.create())
    // ensure in BCS App
    const pubkeyRes = await bcs.getWalletPublicKey(Ledger.defaultPath)
    if (pubkeyRes.bitcoinAddress[0] !== 'Q') {
      throw 'Not BCS App'
    }
    return new Ledger(bcs)
  }
  ...
  static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const pubkeyRes = await ledger.bcs.getWalletPublicKey(path)
    ...
    const inputs = []
    const paths = []
    const selectUtxo = bcsJsLib.utils.selectTxs(utxoList, amount, fee)
    const rawTxCache = {}
    for(let i = 0; i < selectUtxo.length; i++) {
      ...
      inputs.push([
        await ledger.bcs.splitTransaction(rawTxCache[item.hash]),
        item.pos
      ])
    }
    const outputs = new bcsJsLib.TransactionBuilder(keyPair.network)
    ...
    return await ledger.bcs.createPaymentTransactionNew(inputs, paths, undefined, outputsScript)
  }
  ...
  static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const outputs = new bcsJsLib.TransactionBuilder(keyPair.network)
    const contract =  bcsJsLib.script.compile([
      OPS.OP_4,
